...
export default function CodingChallenge({ title, description, initialCode, answer, hints = [], keywords, preview = false, previewType = 'terminal' }: CodingChallengeProps) {
  ...
  return (
    <div className="rounded-xl border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20 p-6 my-6">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-8 h-8 rounded-lg bg-emerald-500 flex items-center justify-center"><Code2 className="text-white" size={16} /></div>
        <span className="text-sm font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">コーディングチャレンジ</span>
      </div>
      ...
      {showHint && hints.length > 0 && (
        <div className="px-4 py-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 mb-4">
          <div className="flex items-center gap-2 mb-1"><Lightbulb size={14} className="text-amber-600 dark:text-amber-400" /><span className="text-xs font-semibold text-amber-700 dark:text-amber-300">ヒント {hintIndex + 1} / {hints.length}</span></div>
          <p className="text-sm text-foreground/80">{hints[hintIndex]}</p>
        </div>
      )}
      {showAnswer && (
        <div className="rounded-lg overflow-hidden border border-border bg-[#1e1e2e] mb-4">
          <div className="px-4 py-2 bg-[#181825] border-b border-[#313244]"><span className="text-xs font-mono text-emerald-400 uppercase">模範解答</span></div>
          <pre className="p-4 font-mono text-sm leading-relaxed text-[#cdd6f4] overflow-x-auto whitespace-pre">{answer}</pre>
        </div>
      )}
      <div className="flex flex-wrap gap-2">
        <button onClick={handleCheck} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition-colors">チェックする</button>
        {hints.length > 0 && (<button onClick={handleNextHint} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-300 text-sm hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors"><Lightbulb size={14} />{showHint && hintIndex < hints.length - 1 ? '次のヒント' : 'ヒントを見る'}</button>)}
        <button onClick={() => setShowAnswer(!showAnswer)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-border text-muted-foreground text-sm hover:bg-muted transition-colors">{showAnswer ? <EyeOff size={14} /> : <Eye size={14} />}{showAnswer ? '模範解答を隠す' : '模範解答を見る'}</button>
        <button onClick={handleReset} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"><RotateCcw size={14} />リセット</button>
      </div>
    </div>
  );
}
